        ...
        if ($productsize < 2 * 1024 * 1024) { // Less than 2 MB
            if (in_array($ext, ['jpg', 'png', 'jpeg', 'gif'])) {
                if (move_uploaded_products($_productsS['dataproducts']['tmp_name'], '../uploads/' . $finalname)) {
                    $insert = "INSERT INTO products (title, description, image, price, rating_id, qty, discount, status, created_at, updated_at) VALUES ('$title', '$description', '$finalname', '$price', '$rating_id', '$qty', '$discount', 1, NOW(), NOW())";
